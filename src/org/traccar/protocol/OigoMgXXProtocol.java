...
public class OigoMgXXProtocol extends BaseProtocolDecoder {
    ...
    public byte[] getHandlePacket(byte pktBytes[])
    {
        if(pktBytes != null && pktBytes.length > 0)
        {
            ...
            Print.logInfo((new StringBuilder()).append("Recv[HEX]: ").append(hex).toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("Recv[Length]: ").append(pktBytes.length).toString(), new Object[0]);
            byte rtn[] = null;
    ...
    private byte[] parseInsertRecord_MGXX(String hex)
    {
        ...
        Print.logInfo("Parsing(Oigo MG-XX Series)", new Object[0]);
        int data_ext = 0;
        String start_of_frame = "";
        String gps_rssi = "00";
        String sequence_counter = "00";
        String version_id = "00";
        ...
        if(hex.length() >= 74)
        {
            ...
            String utc_time = hex.substring(34 + data_ext, 42 + data_ext);
            String gps_latitude = hex.substring(42 + data_ext, 50 + data_ext);
            ...
            if(hex.length() >= 76)
                gps_rssi = hex.substring(74 + data_ext, 76 + data_ext);
            if(hex.length() >= 78)
            {
                sequence_counter = hex.substring(76 + data_ext, 78 + data_ext);
                version_id = hex.substring(78 + data_ext, 80 + data_ext);
            }
            if(hex.length() >= 82)
                gps_satellites_status = hex.substring(80 + data_ext, 82 + data_ext);
            if(hex.length() >= 90)
                virtual_odometer = hex.substring(82 + data_ext, 90 + data_ext);
            if(gps_latitude.equalsIgnoreCase("00000000"))
            {
                Print.logInfo("Latitud Invalida, Se Interrumpe Trama", new Object[0]);
                return null;
            }
            if(gps_longitude.equalsIgnoreCase("00000000"))
            {
                Print.logInfo("Longitud Invalida, Se Interrumpe Trama", new Object[0]);
                return null;
            }
            ...
            if(flag_bin.length() == 2)
                flag_inputs = (new StringBuilder()).append("00").append(flag_bin).toString();
            else
            if(flag_bin.length() == 3)
                flag_inputs = (new StringBuilder()).append("0").append(flag_bin).toString();
            else
            if(flag_bin.length() == 4)
                flag_inputs = flag_bin;
            if(flag_inputs.substring(0, 1) == "1")
                need_ack = true;
            String gpio_direction_bin = hex2BinStr(gpio_direction);
            String gpio_direction_inputs = "00000000";
            if(gpio_direction_bin.length() == 1)
                gpio_direction_inputs = (new StringBuilder()).append("0000000").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 2)
                gpio_direction_inputs = (new StringBuilder()).append("000000").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 3)
                gpio_direction_inputs = (new StringBuilder()).append("00000").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 4)
                gpio_direction_inputs = (new StringBuilder()).append("0000").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 5)
                gpio_direction_inputs = (new StringBuilder()).append("000").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 6)
                gpio_direction_inputs = (new StringBuilder()).append("00").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 7)
                gpio_direction_inputs = (new StringBuilder()).append("0").append(gpio_direction_bin).toString();
            else
            if(gpio_direction_bin.length() == 8)
                gpio_direction_inputs = gpio_direction_bin;
            String gps_satellites_status_bin = hex2BinStr(gps_satellites_status);
            String gps_satellites_status_inputs = "00000000";
            if(gps_satellites_status_bin.length() == 1)
                gps_satellites_status_inputs = (new StringBuilder()).append("0000000").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 2)
                gps_satellites_status_inputs = (new StringBuilder()).append("000000").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 3)
                gps_satellites_status_inputs = (new StringBuilder()).append("00000").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 4)
                gps_satellites_status_inputs = (new StringBuilder()).append("0000").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 5)
                gps_satellites_status_inputs = (new StringBuilder()).append("000").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 6)
                gps_satellites_status_inputs = (new StringBuilder()).append("00").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 7)
                gps_satellites_status_inputs = (new StringBuilder()).append("0").append(gps_satellites_status_bin).toString();
            else
            if(gps_satellites_status_bin.length() == 8)
                gps_satellites_status_inputs = gps_satellites_status_bin;
            gps_heading = hex2DecimalStr(gps_heading);
            gps_speed = hex2DecimalStr(gps_speed);
            gps_rssi = hex2DecimalStr(gps_rssi);
            ...
            long fixtime = _parseDateOigo(utc_time, time_seconds);
            double latitude = _parseLatitudeOigo(gps_latitude);
            ...
            double rssi = StringTools.parseDouble(gps_rssi, 0.0D);
            int satsCount = bin2DecimalInt(gps_satellites_status_inputs.substring(4, 8));
            ...
            Print.logInfo((new StringBuilder()).append("IMEI\t\t: ").append(modemID).toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("Timestamp\t: ").append(fixtime).append(" [").append(new DateTime(fixtime)).append("]").append("\n").toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("UniqueID\t: ").append(uniqueID).toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("DeviceID\t: ").append(accountID).append("/").append(deviceID).toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("Evento\t: ").append(report_tag).toString(), new Object[0]);
            Print.logInfo((new StringBuilder()).append("Secuencia\t: ").append(hex2DecimalStr(sequence_counter)).toString(), new Object[0]);
            if(report_tag.equalsIgnoreCase("01"))
                statusCode = 64793;
            else
            if(!report_tag.equalsIgnoreCase("02"))
                if(report_tag.equalsIgnoreCase("03"))
                    statusCode = 61722;
                else
                if(report_tag.equalsIgnoreCase("09"))
                    statusCode = 64787;
                else
                if(report_tag.equalsIgnoreCase("0F"))
                    statusCode = 64789;
                else
                if(report_tag.equalsIgnoreCase("12"))
                    statusCode = 62465;
                else
                if(report_tag.equalsIgnoreCase("13"))
                    statusCode = 62467;
                else
                if(!report_tag.equalsIgnoreCase("14"))
                    if(report_tag.equalsIgnoreCase("15"))
                        statusCode = 61727;
                    else
                    if(report_tag.equalsIgnoreCase("19"))
                        statusCode = 61714;
                    else
                    if(report_tag.equalsIgnoreCase("1A"))
                        statusCode = 61715;
                    else
                    if(report_tag.equalsIgnoreCase("1C"))
                        statusCode = 61720;
            if(statusCode == 61472)
            {
            ...
            if(parseInsertRecord_Common(gpsEvent))
            {
                ...
                {
                    String ack_start = "E0";
                    String ack_counter = sequence_counter;
                    String ack = (new StringBuilder()).append(ack_start).append(ack_counter).toString();
                    Print.logInfo((new StringBuilder()).append("Respuesta ACK [").append(modemID).append("]\t: ").append(ack).toString(), new Object[0]);
                    return ack.getBytes();
                } else
                {
                    Print.logInfo((new StringBuilder()).append("Sin ACK [").append(modemID).append("]").toString(), new Object[0]);
                    return null;
                }
            } else
            {
    ...
    private boolean parseInsertRecord_Common(GPSEvent gpsEv)
    {
        ...
        Print.logInfo((new StringBuilder()).append("GPS      \t: ").append(geoPoint).toString(), new Object[0]);
        ...
        Print.logInfo((new StringBuilder()).append("Odometer KM: ").append(odomKM).toString(), new Object[0]);
        ...
        if(SIMEVENT_GEOZONES && geoPoint.isValid())
        {
            ...
            if(zone != null)
            {
                ...
                for(Iterator i$ = zone.iterator(); i$.hasNext(); Print.logInfo((new StringBuilder()).append("Geozone    : ").append(z).toString(), new Object[0]))
                {
                ...
                if(chgMask != 0L)
                {
                    for(int b = 0; b <= 15; b++)
                    {
                        ...
                        if((chgMask & m) != 0L)
                        {
                            ...
                            Print.logInfo((new StringBuilder()).append("GPIO : ").append(StatusCodes.GetDescription(inpCode, null)).toString(), new Object[0]);
                        }
                    }
